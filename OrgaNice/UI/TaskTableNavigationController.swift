...
class TaskTableNavigationController: UINavigationController {
	...
	private func setupSideMenu() {
		...
		let priPolButton = SideMenuButton(frame: priPolFrame, title: NSAttributedString(string: "  ?", attributes: [NSAttributedString.Key.font: UIFont(name: "Helvetica Neue", size: 45)!]), image: nil, color: Utils.getTaskCellColor(priority: Task.PRIORITY_MIN + 20)) { (sender) in
